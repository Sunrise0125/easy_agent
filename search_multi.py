...
def _generate_arxiv_pdf_url(landing_url: Optional[str]) -> Optional[str]:
    """
    Generate arXiv PDF URL from abstract page URL.
    
    Pattern: https://arxiv.org/abs/XXXX.XXXXX → https://arxiv.org/pdf/XXXX.XXXXX.pdf
    
    Args:
        landing_url: arXiv abstract page URL
        
    Returns:
        PDF download URL if pattern matches, None otherwise
    """
    if not landing_url:
        return None
    
    # Check if URL contains arXiv abstract pattern
    if "arxiv.org/abs/" in landing_url:
        return landing_url.replace("/abs/", "/pdf/") + ".pdf"
...
async def _search_s2_single_query(
    ...
    while pages < max_pages:
        ...
        if len(collected_no_client_filter) >= intent.max_results:
            logger.info(f"[S2] collected {len(collected_no_client_filter)}, reached max_results={intent.max_results}, stop early")
            break
    ...
    try:
        ...
        for e in entries:
            ...
            pm = PaperMetadata(
                ...
                journal="arXiv",
                url=id_url,
                pdf_url=_generate_arxiv_pdf_url(id_url),
...
async def _safe_invoke_callback(
    callback: Optional[Callable[[str, str, int, Optional[int], Optional[List[str]]], Awaitable[None]]],
    source: str,
    status: str,
    fetched: int = 0,
    ...
    """
    Safely invoke progress callback, catching and logging any exceptions.
    ...
    try:
        await callback(source, status, fetched, total, errors)
    except Exception as e:
        logger.warning(f"Progress callback failed for source {source}: {e}")
...
async def search_papers(
    intent: SearchIntent,
    progress_callback: Optional[Callable[[str, str, int, Optional[int], Optional[List[str]]], Awaitable[None]]] = None
) -> Tuple[List[PaperMetadata], Dict[str, Any]]:
    """
    ...
    Args:
        intent: Search intent with query parameters
        progress_callback: Optional async callback for progress updates
            Signature: async def callback(source: str, status: str, fetched: int, total: Optional[int], errors: Optional[List[str]])
    """
    # ---------- 允许的来源 & 选择 ----------
    ...
    # ---------- 查询组合、分页 ----------
    queries = _build_query_combinations(intent)
    # No hard cap on per_page; source-specific limits applied in each search function
    per_page = max(intent.max_results, 50)
    # Dynamic max_pages: 1.5x buffer for deduplication, capped at 10
    max_pages = min(ceil(intent.max_results * 1.5 / per_page), 10)
    ...
    for i, query in enumerate(queries, 1):
        ...
        for src in selected_sources:
            ...
            await _safe_invoke_callback(progress_callback, src, "in_progress", 0, None, None)
            ...
            try:
                ...
                await _safe_invoke_callback(
                    progress_callback, src, "completed",
                    fetched=st["after_filter"], total=None, errors=None
                )
                ...
                await _safe_invoke_callback(
                    progress_callback, src, "failed",
                    fetched=0, total=None, errors=[str(e)]
                )
