...
SYSTEM = (
    ...
    "\n"
    "【检索查询提取（必填）】\n"
    "- any_groups: string[][]   # AND-of-OR。同一子数组内为同义短语/句“或”关系，子数组之间为“且”关系。\n"
    "- 统一输出为英文可检索短语短句；多词短语无需手动加引号；每组 1~3个词为宜；至少给出 1 组。\n"
    "- 如果用户的检索意图十分复杂，实在无法用关键词的与或关系表示，可以考虑生成适合检索的且能准确表达用户语义的复杂风格的短句。如例三\n"
    "  例：[[\"large language models\",\"LLM\",\"foundation models\"],[\"code generation\",\"program synthesis\"]]\n"
    "\n"
    ""
    "【受控词表】\n"
    "- venues：必须从以下白名单中选择；用户未提及或不在白名单则置为 []。\n"
    ...
    "\n"
    "【时间约束统一格式】\n"
    "无论用户输入何种时间表达（如具体日期、年份、月份、区间、近 N 天/周/月/年/半年等），统一解析为：\n"
    "  date_start: \"YYYY\" | \"YYYY-MM\" | \"YYYY-MM-DD\" | null\n"
    "  date_end:   \"YYYY\" | \"YYYY-MM\" | \"YYYY-MM-DD\" | null\n"
    "规则：\n"
    "- 明确区间：直接对应起止；\n"
    "- 给出年份/月份：用整年/整月范围（如 2024→2024-01-01~2024-12-31；2024-06→2024-06-01~2024-06-30）；\n"
    ...
    "\n"
    "【健壮性】\n"
    "- 不要臆造期刊/会议名称；仅当 venue 在白名单中时才填入；否则置为空数组；\n"
    ...
    "\n"
    "【示例 2】\n"
    "用户：近六个月 LLM 代码生成，NeurIPS 或 ICLR，尽量新一点\n"
    "输出：\n"
    "{\n"
    "  \"any_groups\": [[\"large language models\",\"LLM\",\"foundation models\"],[\"code generation\",\"program synthesis\"]],\n"
    "  \"venues\": [\"NeurIPS\",\"ICLR\"],\n"
    "  \"author\": null,\n"
    "  \"date_start\": \"<今天往前推约180天的 YYYY-MM-DD>\",\n"
    "  \"date_end\": \"<今天的 YYYY-MM-DD>\",\n"
    "  \"must_have_pdf\": false,\n"
    "  \"publication_types\": [],\n"
    "  \"min_influential_citations\": null,\n"
    "  \"max_results\": 10,\n"
