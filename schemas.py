...
class PaperMetadata(BaseModel):
    ...
    pdf_url: Optional[str] = None  # PDF 直接下载 URL
...
class SourceProgress(BaseModel):
    """Progress tracking for a single search source"""
...
class TaskProgress(BaseModel):
    """Overall task progress information"""
    stage: str
    stage_description: str = ""  # 中文描述，如"正在解析查询意图"、"进入排序阶段"等
    sources: Dict[str, SourceProgress] = Field(default_factory=dict)
    overall_percent: int = 0

class TaskState(BaseModel):
    """Complete task state for async search tasks"""
    task_id: str
    status: TaskStatus
...
class SearchTaskRequest(BaseModel):
    """Request body for creating a search task"""
    user_query: str = Field(..., max_length=500, description="Natural language search query")
